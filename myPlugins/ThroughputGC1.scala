...
def metInit(tab:Array[String]=null) {
    ...
    ThroughputGC1.oldDateInMillis=0L
  }
  def retour(tabStr: Array[String]): Double =
    {
		...
		if (tabStr(0).contains("GC pause (young)")) {
			// System.out.println("GC pause (young)");
			pat = "\\[Parallel Time:\\s*\\d+\\.\\d*\\s+ms\\]".r
		...
		if (tabStr(0).contains("[Full GC (System.gc())")) {
			// System.out.println("Full GC");
			pat ="\\[Full GC \\(System\\.gc\\(\\)\\).+?\\d+\\.\\d*".r
			match0 = pat.findFirstIn(tabStr(0))
			if (None != match0) {
				found = true;
				var ext1 = match0.get
		...
		var  res:Double = 100 * (1 - (val0.toDouble * echelle)
				/  (newDate - ThroughputGC1.oldDateInMillis).toDouble);
		ThroughputGC1.oldDateInMillis = newDate;
