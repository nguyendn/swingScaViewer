	...
	//	println("tabStr0="+tabStr(0))
		...
		if (None != ext1) {
		  ...
		  // println("ext1Str="+ext1Str)
		   ...
		   val idx=tabStr(0).indexOf(ext1Str)
			val newStr=tabStr(0).substring(idx+ext1Str.length())
			//println("newStr="+newStr)
			val ext2=regex1.findFirstIn(newStr)
			if (None != ext2) {
				val ext2Str= ext2.get
				//println("ext2Str="+ext2Str)
				date2= sdf.parse( ext2Str).getTime
			} else {
				return Double.NaN
			}
		} else {
